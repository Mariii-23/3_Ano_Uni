...
void renderCatmullRomCurve() {
  ...
  for (float gt = 0.0; gt < 1; gt += 1.0 / POINTS) {
    getGlobalCatmullRomPoint(gt, position, derivative);
...
void renderScene(void) {
  ...
  getGlobalCatmullRomPoint(TIME, position, derivative);
  glTranslatef(position[0], position[1], position[2]);

  float x[3] = {derivative[0], derivative[1], derivative[2]};
  normalize(x);
  float z[3];
  cross(x, prev_y, z);
  normalize(z);
  ...
  glutWireCone(SIZE_TEAPOT, 2 * SIZE_TEAPOT, 16, 16);
  glPopMatrix();
