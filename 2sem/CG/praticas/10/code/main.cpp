...
void prepareTerrain() {
  ...
  for (int i = 1; i < imageWidth - 2; i++) {
    for (int j = 1; j < imageWidth - 1; j++) {
      ...
      // fill texCoord vector with the texture coordinates for grid location
      // (i+1,j)
      // texCoord.push_back(i + 1);
      // texCoord.push_back(j);

      texCoord.push_back(i - imageWidth * 0.5f + 1);
      texCoord.push_back(h(i + 1, j));
  ...
  glBufferData(GL_ARRAY_BUFFER, position.size() * sizeof(float), &(position[0]),
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, texCoord.size() * sizeof(float), &(texCoord[0]),
               GL_STATIC_DRAW);
